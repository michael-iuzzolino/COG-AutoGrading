...
class GradeSubmission():
    ...
    def __init__(self, submission_directory, COG_script_directory, local_test=False):
        ...
        if not found_all_functions:
            studentFeedback("\tERROR: Could not find all required functions.")
            return
            # EXIT
        studentFeedback("\tSuccess!")
        ...
        self.compile(self.helper_file_path, helper_compiled_name)
        studentFeedback("\tSuccess!")


        """
        Step 5: Generate Data for helper.cpp test cases
        """
    ...
    def locate_submission(self):
        ...
        for expected_file in self.expected_files:
            studentFeedback("\tSearching for '{}'...".format(expected_file), new_line=False)
            ...
            if submission_file_name:
                ...
                studentFeedback("\tFound file '{}'!\n".format(submission_file_name))
            ...
            # File not found
                ...
                print("\tFile {} not found.".format(expected_file))
    ...
    def write_helper(self):
        ...
        if self.local_system_test:
            ...
            self.defunct_student_file_path = "helper/student_functions_file.cpp"
            return
        ...
        self.defunct_student_file_path = self.submission_directory_name + "/student_functions_file.cpp"
    ...
    def create_altered_student_file(self):
        ...
        for assignment_path, assignment_name in zip(self.student_files_path, self.student_file_names):
            ...
            with open(assignment_path, "r") as student_file, open(self.defunct_student_file_path, "w") as defunct_student_file:
                ...
                for line in student_file:
                    ...
                    if ("int main()".replace(" ","") in line.lower().replace(" ", "")) or ("int main(void)".replace(" ", "") in line.lower().replace(" ", "")):
                        line = line.replace("main", "defunct")
    ...
    def generate_test_data(self):
        ...
        self.test_output = [out1, out2, out3, out4, out5]
        ...
        in3 = "0"
        in4 = "100"
        self.test_input = [in1, in2, in3, in4]


    def parse_student_answers(self, student_answers):
        catch_phrases = ["The population is",
                         "The time is",
                         "degrees celsius is"]
        answers_string = " ".join(student_answers)

        answers_list = re.split(r'[.:]+', answers_string)
    ...
    def test_cases(self, helper_path):
        ...
        if num_student != num_expected:
            number_wrong = math.fabs(num_student - num_expected)
            lost_point = number_wrong * 5
        ...
        for index, (expected_answer, student_answer) in enumerate(zip(expected_outputs, student_answers)):
            ...
            if index % 2 == 0:
                function_num += 1
            studentFeedback("{:6s}Checking cout output from your function, '{:s}'...".format("", self.function_names[function_num]))
    ...
    def check_output(self, solution_outfile, student_outfile, test_number):
        ...
        points_lost_per_question = 1
        for expected_line, student_line in zip(solution_outfile, student_outfile):

            if expected_line != student_line:
                studentFeedback("{:2s} \t {:20s} \t {:20s} {:2s} ".format("**", expected_line.strip(), student_line.strip(), "**  INCORRECT"))
                self.make_deduction(points_lost_per_question, "Incorrect output. Expected output {} vs. Your Output: {}".format(expected_line, student_line))
            else:
    ...
    def check_functions(self):
        ...
        self.function_names = ["calculatePopulation", "secondsToTime", "celsiusToFahrenheit"]
        # Define the required functions in their full name (for printing later)
        required_functions = ["void calculatePopulation(void)",
        ...
        check_required_functions = [self.split_function_string(func.lower()) for func in required_functions]
        ...
        student_file_content = self.student_files_content[0]
        ...
        for line in student_file_content:
            # Parse the line
            parsed_line = self.split_function_string(line.strip().lower())
            ...
            for required_function in check_required_functions:
                ...
                for full_req_fnx in required_functions:
                    ...
                    if ' '.join(required_function) == full_req_fnx_check:
                        self.make_deduction(10, "{:s} function is required, but is missing from your program. Add this function for full credit.".format(full_req_fnx))
